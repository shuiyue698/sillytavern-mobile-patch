...
router.post('/test-image-connection', async (request, response) => {
    try {
        const baseUrl = String(request.body?.api_url || 'https://dapi.touchmyai.online/v1').replace(/\/+$/, '');
        ...
        const result = await fetch(`${baseUrl}/models`, { headers: { Authorization: `Bearer ${key}` } });
        const text = await result.text();
        if (!result.ok) return response.status(result.status).send(text || 'Connection failed');
        let data;
...
router.post('/caption-image', async (request, response) => {
    try {
        ...
        let headers = {};
        let bodyParams = {};
        let useConfiguredImageRelay = false;

        // The mobile image panel may provide its own relay credentials. Prefer
        // those over server-side secrets so reference analysis uses the same
        // endpoint as image generation.
        const suppliedApiKey = String(request.body?.api_key || '').trim();
        const suppliedApiUrl = String(request.body?.api_url || '').trim().replace(/\/+$/, '');

        if (request.body.api === 'openai' && !request.body.reverse_proxy) {
            key = suppliedApiKey || readSecret(request.user.directories, SECRET_KEYS.OPENAI);
            if (!key) {
                key = readSecret(request.user.directories, SECRET_KEYS.CUSTOM);
                useConfiguredImageRelay = !!key;
            }
        }
        ...
        if (request.body.api === 'openai') {
            apiUrl = suppliedApiUrl
                ? `${suppliedApiUrl}/chat/completions`
                : useConfiguredImageRelay
                ? 'https://dapi.touchmyai.online/v1/chat/completions'
...
router.post('/generate-image', async (request, response) => {
    try {
        // Reuse the configured OpenAI-compatible relay for image generation.
        const key = String(request.body?.api_key || '').trim()
            || readSecret(request.user.directories, SECRET_KEYS.CUSTOM)
            || readSecret(request.user.directories, SECRET_KEYS.OPENAI);

        if (!key) {
            console.warn('No OpenAI-compatible API key found');
            return response.sendStatus(400);
        }

        console.debug('OpenAI request', request.body);

        const referenceImage = request.body?.reference_image;
        const baseUrl = String(request.body?.api_url || 'https://dapi.touchmyai.online/v1').replace(/\/+$/, '');
        ...
        if (referenceImage) {
            ...
            const match = String(referenceImage).match(/^data:([^;]+);base64,(.+)$/);
            const mime = match?.[1] || 'image/png';
            const encoded = match?.[2] || referenceImage;
            form.append('image', Buffer.from(encoded, 'base64'), {
                filename: 'reference.png',
                contentType: mime,
            });
            form.append('prompt', String(request.body.prompt || ''));
            form.append('model', String(request.body.model || 'gpt-image-1'));
            form.append('size', String(request.body.size || '1024x1024'));
            form.append('n', String(request.body.n || 1));
            body = form;
