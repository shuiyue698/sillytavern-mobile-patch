...
// Lightweight keyless web search fallback for the chat input tool.
router.post('/quick', async (request, response) => {
    try {
        const query = String(request.body?.query || '').trim();
        if (!query) return response.status(400).json({ error: 'Query is required' });
        const result = await fetch(`https://html.duckduckgo.com/html/?q=${encodeURIComponent(query)}`, { headers: visitHeaders });
        if (!result.ok) return response.status(502).json({ error: 'Search provider unavailable' });
        const html = await result.text();
        const items = [...html.matchAll(/result__a[^>]*href="([^"]+)"[^>]*>([\s\S]*?)<\/a>[\s\S]*?result__snippet[^>]*>([\s\S]*?)<\/a?/gi)]
            .slice(0, 8)
            .map(match => ({
                url: decode(match[1]),
                title: decode(match[2].replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim()),
                snippet: decode(match[3].replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim()),
            }));
        return response.json({ query, results: items });
    } catch (error) {
        console.error('Quick web search failed', error);
