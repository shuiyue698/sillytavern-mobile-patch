...
export async function init() {
    async function addExtensionControls() {
        ...
        $('#tts_use_gpt_sovits').on('click', async () => {
            const provider = 'GPT-SoVITS-V2 (Unofficial)';
            if (!ttsProviders[provider]) return;
            extension_settings.tts.currentProvider = provider;
            saveSettingsDebounced();
            $('#tts_provider').val(provider);
            await loadTtsProvider(provider);
        });
