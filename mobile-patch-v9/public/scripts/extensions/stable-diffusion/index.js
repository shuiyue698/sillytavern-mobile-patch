...
const modeLabels = {
    ...
    [generationMode.PROTAGONIST]: 'Current Protagonist',
...
const triggerWords = {
    ...
    [generationMode.PROTAGONIST]: ['protagonist'],
...
function getQuietPrompt(mode, trigger) {
    if (mode === generationMode.FREE || mode === generationMode.PROTAGONIST) {
        return trigger;
    }

    const prompt = stringFormat(extension_settings.sd.prompts[mode], trigger);
    if (mode === generationMode.USER) {
        return `${prompt} Preserve the protagonist's explicitly established gender, age, and visual traits from the chat. Do not invent a different person or change the protagonist into a male character.`;
    }
...
function getRawLastMessage() {
    const paragraphs = getLatestStoryParagraphs();
    const protagonist = String(extension_settings.sd.protagonist_override || '').trim();
    const scene = String(extension_settings.sd.scene_cast_override || '').trim();
    const settings = [
        protagonist && `Authoritative protagonist setting:\n${protagonist}`,
        scene && `Authoritative scene cast setting:\n${scene}`,
    ].filter(Boolean);
    return [
        'Illustrate the current story faithfully from this final paragraph.',
        'Only depict people, clothing, actions, objects, and setting explicitly supported by the text.',
        'The protagonist is female. Never add a male protagonist, fantasy armor, weapons, or unrelated characters.',
        ...settings,
        'Final paragraph:',
        paragraphs.at(-1),
    ].join('\n');
...
function getLatestStoryParagraphs() {
    const context = getContext();
    for (const message of context.chat.slice().reverse()) {
        if (message.is_system || message.is_user || Array.isArray(message.extra?.media)) {
            continue;
        }

        const content = String(message.mes || '').match(/<content>([\s\S]*?)<\/content>/i)?.[1] || String(message.mes || '');
    ...
    toastr.warning('No recent story message was found in the chat.', 'Image Generation');
    throw new Error('No recent story message found.');
}

function getCharacterCardPrompt() {
    const context = getContext();
    const character = context.characters?.[context.characterId];
    const data = character?.data || character || {};
    return [data.name || character?.name, data.description, data.personality, data.scenario]
        .filter(value => typeof value === 'string' && value.trim())
        .join('\n');
}

function getProtagonistPrompt() {
    const override = String(extension_settings.sd.protagonist_override || '').trim();
    const paragraphs = getLatestStoryParagraphs();
    const protagonistParagraphs = paragraphs.filter(x => /你|乐正绫/.test(x));
    const details = override ? [override] : (protagonistParagraphs.length > 0 ? protagonistParagraphs : paragraphs).slice(-4);
    const card = getCharacterCardPrompt();

    return [
        'Create a full-body portrait of the story protagonist only.',
        'The protagonist is the person addressed as "你". She is female. Never depict a male character or a different person.',
        'Show her complete head, face, body, hands, legs, and both feet in frame. Use a pulled-back camera, centered full-body composition, with generous space above the head and below the feet. Never crop the head, hands, or feet.',
        'Preserve every explicitly stated visual trait. Use a neutral studio background unless the narrative specifies an essential visual setting.',
        card && extension_settings.sd.source_mode !== 'interface_text' ? `Character card details:\n${card}` : '',
        'Authoritative narrative details:',
        details.join('\n'),
    ].join('\n');
}

function getScenePrompt() {
    const cast = String(extension_settings.sd.scene_cast_override || '').trim() || 'the female protagonist and only the NPCs explicitly present in the narrative';
    const source = extension_settings.sd.source_mode === 'interface_text'
        ? getLatestStoryParagraphs().join('\n')
        : `Character card and current story:\n${getCharacterCardPrompt()}\n${getLatestStoryParagraphs().join('\n')}`;
    return [
        'Create a cinematic wide shot of the current story scene.',
        `Depict the location, lighting, objects, and atmosphere from the narrative. The only characters allowed in the scene are: ${cast}.`,
        'Never substitute unrelated people, male protagonists, fantasy armor, weapons, or a different setting.',
        'Authoritative narrative details:',
        source,
    ].join('\n');
}
 ...
 */
/** Analyze the uploaded reference with the existing multimodal caption endpoint. */
async function analyzeReferenceImage(dataUrl) {
    const status = $('#sd_gen_reference_status');
    if (!dataUrl || !dataUrl.startsWith('data:image/')) return '';
    status.text('Analyzing reference image...');
    try {
        const analysisPrompt = [
            'Analyze this reference image for image generation.',
            'Describe the visible subjects and identity-defining details: gender presentation, age range, face, hair, eyes, skin tone, body shape, clothing, accessories, pose, camera framing, background, lighting, colors, and art style.',
            'Be concrete and concise. Do not invent details that are not visible. Return one clean paragraph in English.'
        ].join(' ');
        let caption = '';
        let lastError = null;
        const selectedModel = String(extension_settings.sd.reference_analysis_model || $('#sd_gen_analysis_model').val() || '').trim();
        const models = [selectedModel, 'gpt-5.6-luna', 'gpt-5.6-sol', 'gpt-5.6-terra', 'gpt-4o', 'gpt-4-turbo', 'gpt-4.1-mini']
            .filter(model => model && !/^gpt-image-/.test(model))
            .filter((model, index, list) => list.indexOf(model) === index);
        for (const model of models) {
            const controller = new AbortController();
            const timeout = setTimeout(() => controller.abort(), 60000);
            try {
                const result = await fetch('/api/openai/caption-image', {
                    method: 'POST',
                    ...
                    body: JSON.stringify({
                        ...
                        // Reuse the active Chat Completions custom endpoint
                        // from the left API panel when it is selected.
                        api: oai_settings?.chat_completion_source === 'custom' ? 'custom' : 'openai',
                        model,
                        api_url: extension_settings.sd.image_api_url || '',
                        api_key: extension_settings.sd.image_api_key || '',
                        server_url: oai_settings?.custom_url || '',
                    }),
                });
                if (!result.ok) throw new Error(await result.text() || `HTTP ${result.status}`);
        ...
        if (!caption && lastError) throw lastError;
        const cleanCaption = String(caption || '').trim().slice(0, 8000);
        extension_settings.sd.reference_caption = cleanCaption;
        $('#sd_gen_reference_caption').val(cleanCaption || '分析接口未返回文字，请检查中转站是否提供视觉聊天模型。');
        saveSettingsDebounced();
        status.text(cleanCaption ? 'Reference analysis ready' : 'No analysis returned');
        return cleanCaption;
    } catch (error) {
        console.error('Reference image analysis failed:', error);
        extension_settings.sd.reference_caption = '';
        const fallbackParts = [];
        const protagonist = String(extension_settings.sd.protagonist_override || '').trim();
        const scene = String(extension_settings.sd.scene_cast_override || '').trim();
        if (protagonist) fallbackParts.push(`Protagonist details from the left panel: ${protagonist}`);
        if (scene) fallbackParts.push(`Scene and cast details from the left panel: ${scene}`);
        const fallback = fallbackParts.join('\n') || 'Describe the uploaded reference image faithfully and preserve its visible subject, clothing, pose, composition, lighting and colors.';
        extension_settings.sd.reference_caption = fallback;
        saveSettingsDebounced();
        $('#sd_gen_reference_caption').val(`${fallback}\n\n[Automatic image analysis unavailable: ${error.message || 'upstream error'}]`);
        status.text('Automatic analysis unavailable; editable draft created');
        return '';
    }
}
...
async function generatePicture(initiator, args, trigger, message, callback) {
    ...
    try {
        ...
        const manualPrompt = String(extension_settings.sd.manual_prompt || '').trim();
        if (manualPrompt) {
            prompt = `${prompt}\nAdditional user instructions (must follow): ${manualPrompt}`;
        }
        const stylePrompt = String(extension_settings.sd.style_prompt || '').trim();
        if (stylePrompt) {
            prompt = `${prompt}\nVisual style: ${stylePrompt}.`;
        }
        const referenceCaption = String(extension_settings.sd.reference_caption || '').trim();
        if (referenceCaption) {
            prompt = `${prompt}\nReference image analysis (preserve identity, clothing, pose, composition and visual style): ${referenceCaption}`;
        }
...
async function getPrompt(generationType, message, trigger, quietPrompt, combineNegatives) {
    ...
    switch (generationType) {
        case generationMode.PROTAGONIST:
            prompt = getProtagonistPrompt();
            break;
        case generationMode.SCENARIO:
            prompt = getScenePrompt();
            break;
        case generationMode.RAW_LAST:
            prompt = getRawLastMessage();
            break;
    ...
    if (![generationMode.FREE, generationMode.PROTAGONIST, generationMode.SCENARIO, generationMode.RAW_LAST].includes(generationType)) {
...
async function sendGenerationRequest(generationType, prompt, additionalNegativePrefix, characterName, callback, initiator, signal) {
    ...
    if (extension_settings.sd.source === sources.openai && result.format === 'png') {
        result.data = await resizeOpenAiImage(result.data, extension_settings.sd.width, extension_settings.sd.height);
    }
...
/**
 * Fits an OpenAI image into an exact output size without distorting the subject.
 * @param {string} base64Image Image data without a data URL prefix.
 * @param {number} targetWidth Output width.
 * @param {number} targetHeight Output height.
 * @returns {Promise<string>} A PNG image encoded as base64.
 */
async function resizeOpenAiImage(base64Image, targetWidth, targetHeight) {
    const blob = await fetch(`data:image/png;base64,${base64Image}`).then(response => response.blob());
...
async function generateOpenAiImage(prompt, signal) {
    ...
    const apiImageModel = /gpt-image-/.test(String(oai_settings?.openai_model || '')) ? oai_settings.openai_model : '';
    const model = extension_settings.sd.model_override || apiImageModel || extension_settings.sd.model;
    const isDalle2 = /dall-e-2/.test(model);
    const isDalle3 = /dall-e-3/.test(model);
    const isGptImg = /gpt-image-(?:1(?:\.5)?|2|latest)/.test(model);
    const isSora2 = /sora-2/.test(model);
    ...
    if (extension_settings.sd.image_api_url) requestBody.api_url = extension_settings.sd.image_api_url;
    if (extension_settings.sd.image_api_key) requestBody.api_key = extension_settings.sd.image_api_key;
    if (extension_settings.sd.reference_image && isGptImg) {
        requestBody.reference_image = extension_settings.sd.reference_image;
    }
...
async function sendMessage(prompt, image, generationType, additionalNegativePrefix, initiator, prefixedPrompt, format) {
    ...
    const messageText = initiator === initiators.wand ? '' : substituteParamsExtended(template, { char: name, prompt: prompt, prefixedPrompt: prefixedPrompt });
...
async function addSDGenButtons() {
    if (!$('#st_web_search').data('initialized')) {
        const searchButton = $('#st_web_search');
        extension_settings.sd.web_search_enabled = !!extension_settings.sd.web_search_enabled;
        searchButton.toggleClass('web-search-enabled', extension_settings.sd.web_search_enabled);
        searchButton.attr('title', extension_settings.sd.web_search_enabled ? '联网搜索：已开启（点击关闭）' : '联网搜索：已关闭（点击开启）');
        searchButton.data('initialized', true).on('click', async function () {
            extension_settings.sd.web_search_enabled = !extension_settings.sd.web_search_enabled;
            const enabled = extension_settings.sd.web_search_enabled;
            searchButton.toggleClass('web-search-enabled', enabled);
            searchButton.attr('title', enabled ? '联网搜索：已开启（点击关闭）' : '联网搜索：已关闭（点击开启）');
            saveSettingsDebounced();
            if (!enabled) return;
            const text = String($('#send_textarea').val() || '').trim();
            if (!text) {
                toastr.info('联网搜索已开启，请先在输入框输入要查验的内容。', '网络搜索');
                return;
            }
            ...
            try {
                ...
                if (!result.ok) throw new Error(await result.text() || `HTTP ${result.status}`);
                const data = await result.json();
                const lines = (data.results || []).map((item, index) => `${index + 1}. ${item.title}\n${item.snippet}\n${item.url}`);
                const block = `[网络搜索：${text}]\n${lines.join('\n\n') || '未找到结果'}\n[/网络搜索]`;
                const input = $('#send_textarea');
                const current = String(input.val() || '').trim();
                input.val(current ? `${current}\n\n${block}` : block).trigger('input');
            } catch (error) {
                toastr.error(error.message || String(error), '网络搜索失败');
            } finally {
                button.prop('disabled', false);
            }
        });
    }
    if (extension_settings.sd.width !== 1920 || extension_settings.sd.height !== 1080) {
        extension_settings.sd.width = 1920;
        extension_settings.sd.height = 1080;
        saveSettingsDebounced();
    }

    const buttonHtml = await renderExtensionTemplateAsync('stable-diffusion', 'button');

    $('#attachFile').after(buttonHtml);

    const configPanel = $('#sd_gen_config_panel').detach().appendTo('body');
    const savedPanelPosition = extension_settings.sd.panel_position;
    if (savedPanelPosition?.left && savedPanelPosition?.top) {
        configPanel.css({ left: savedPanelPosition.left, top: savedPanelPosition.top });
    }
    $('#sd_gen_config_drag').on('pointerdown', function (event) {
        if ($(event.target).closest('button').length) return;
        const startX = event.clientX;
        const startY = event.clientY;
        const startLeft = configPanel.offset().left;
        const startTop = configPanel.offset().top;
        const move = (moveEvent) => {
            configPanel.css({ left: `${Math.max(0, startLeft + moveEvent.clientX - startX)}px`, top: `${Math.max(0, startTop + moveEvent.clientY - startY)}px` });
        };
        const stop = () => {
            extension_settings.sd.panel_position = { left: configPanel.css('left'), top: configPanel.css('top') };
            saveSettingsDebounced();
    ...
    const imageModels = new Set(['gpt-image-1', 'gpt-image-1.5', 'gpt-image-2']);
    $('#model_openai_select option').each(function () {
        if (/^gpt-image-/.test(String($(this).val()))) imageModels.add(String($(this).val()));
    });
    const modelSelect = $('#sd_gen_model_override').empty();
    for (const model of imageModels) modelSelect.append(new Option(model, model));
    const initialModel = extension_settings.sd.model_override || (/^gpt-image-/.test(String(oai_settings?.openai_model || '')) ? oai_settings.openai_model : 'gpt-image-1.5');
    modelSelect.val(initialModel);
    modelSelect.on('change', function () {
        extension_settings.sd.model_override = String($(this).val()).trim();
        extension_settings.sd.model = extension_settings.sd.model_override;
        saveSettingsDebounced();
    });
    $('#sd_gen_api_url').prop('disabled', false).prop('readOnly', false).on('pointerdown keydown', event => event.stopPropagation()).val(extension_settings.sd.image_api_url || '').on('input', function () {
        extension_settings.sd.image_api_url = String($(this).val()).trim();
        saveSettingsDebounced();
    });
    $('#sd_gen_api_key').prop('disabled', false).prop('readOnly', false).on('pointerdown keydown', event => event.stopPropagation()).val(extension_settings.sd.image_api_key || '').on('input', function () {
        extension_settings.sd.image_api_key = String($(this).val()).trim();
        saveSettingsDebounced();
    });
    $('#sd_gen_api_test').on('click', async function () {
        const button = $(this).prop('disabled', true);
        const status = $('#sd_gen_api_status').text('测试中...');
        try {
            const response = await fetch('/api/openai/test-image-connection', {
                method: 'POST',
                headers: { ...getRequestHeaders(), 'Content-Type': 'application/json' },
                body: JSON.stringify({ api_url: $('#sd_gen_api_url').val(), api_key: $('#sd_gen_api_key').val() }),
            });
            if (!response.ok) throw new Error(await response.text() || `HTTP ${response.status}`);
            const data = await response.json();
            const imageModelSelect = $('#sd_gen_model_override');
            const knownValues = new Set(imageModelSelect.find('option').map(function () { return String($(this).val()); }).get());
            const imageModels = (data.models || []).filter(model => /(?:gpt-image|dall-e|image|flux|stable-diffusion|sdxl|ideogram|recraft|imagen)/i.test(model));
            for (const model of imageModels) {
                if (!knownValues.has(model)) imageModelSelect.append(new Option(model, model));
            }
            if (imageModels.length) {
                imageModelSelect.val(imageModels[0]).trigger('change');
                toastr.success(`已加载 ${imageModels.length} 个生图模型`, '模型列表');
            }
            status.text(`连接成功${data.model_count ? `（${data.model_count} 个模型）` : ''}`);
        } catch (error) {
            status.text(`连接失败：${error.message || error}`);
        } finally {
            button.prop('disabled', false);
        }
    });
    const analysisModels = new Set(['gpt-5.6-luna', 'gpt-5.6-sol', 'gpt-5.6-terra', 'gpt-4o', 'gpt-4-turbo', 'gpt-4.1-mini']);
    $('#model_openai_select option').each(function () {
        const model = String($(this).val() || '').trim();
        if (model && !/^gpt-image-/.test(model)) analysisModels.add(model);
    });
    const analysisModelSelect = $('#sd_gen_analysis_model').empty();
    for (const model of analysisModels) analysisModelSelect.append(new Option(model, model));
    const savedAnalysisModel = extension_settings.sd.reference_analysis_model || 'gpt-5.6-luna';
    analysisModelSelect.val(savedAnalysisModel);
    if (!analysisModelSelect.val()) analysisModelSelect.prop('selectedIndex', 0);
    analysisModelSelect.on('change', function () {
        extension_settings.sd.reference_analysis_model = String($(this).val() || '').trim();
        saveSettingsDebounced();
    });
    $('#model_openai_select').on('change.sdImageModelSync', function () {
        const selected = String($(this).val() || '');
        if (/^gpt-image-/.test(selected)) modelSelect.val(selected).trigger('change');
    });
    $('#sd_gen_model_override').on('input', function () {
        extension_settings.sd.model_override = String($(this).val()).trim();
        if (extension_settings.sd.model_override) extension_settings.sd.model = extension_settings.sd.model_override;
        saveSettingsDebounced();
    });
    const resolutions = ['1920x1080', '1536x1024', '1792x1024', '1280x720', '1024x1024', '1024x1536'];
    const resolutionSelect = $('#sd_gen_resolution_override').empty();
    for (const resolution of resolutions) resolutionSelect.append(new Option(resolution, resolution));
    resolutionSelect.val(`${extension_settings.sd.width || 1920}x${extension_settings.sd.height || 1080}`);
    resolutionSelect.on('change', function () {
        const match = String($(this).val()).match(/^(\d+)x(\d+)$/);
        if (!match) return;
        extension_settings.sd.width = Number(match[1]);
        extension_settings.sd.height = Number(match[2]);
        saveSettingsDebounced();
    });
    $('#sd_gen_manual_prompt').val(extension_settings.sd.manual_prompt || '').on('input', function () {
        extension_settings.sd.manual_prompt = String($(this).val());
        saveSettingsDebounced();
    });
    $('#sd_gen_source_mode').val(extension_settings.sd.source_mode || 'card_text').on('change', function () {
        extension_settings.sd.source_mode = String($(this).val());
        saveSettingsDebounced();
    });
    $('#sd_gen_style').val(extension_settings.sd.style_prompt || '').on('change', function () {
        extension_settings.sd.style_prompt = String($(this).val());
        saveSettingsDebounced();
    });
    $('#sd_gen_reference').on('change', function () {
        const file = this.files?.[0];
        if (!file) return;
        const reader = new FileReader();
        reader.onload = async () => {
            extension_settings.sd.reference_image = String(reader.result);
            extension_settings.sd.reference_caption = '';
            $('#sd_gen_reference_preview').attr('src', extension_settings.sd.reference_image).css('display', 'block');
            saveSettingsDebounced();
            await analyzeReferenceImage(extension_settings.sd.reference_image);
        };
        reader.readAsDataURL(file);
    });
    if (extension_settings.sd.reference_image) {
        $('#sd_gen_reference_preview').attr('src', extension_settings.sd.reference_image).css('display', 'block');
    }
    if (!$('#sd_gen_reference_status').length) {
        $('#sd_gen_reference_preview').after('<div id="sd_gen_reference_status" class="sd_gen_reference_status"></div>');
    }
    $('#sd_gen_reference_caption').val(extension_settings.sd.reference_caption || '').on('input', function () {
        extension_settings.sd.reference_caption = String($(this).val());
        saveSettingsDebounced();
    });
    $('#sd_gen_reference_clear').on('click', function () {
        extension_settings.sd.reference_image = '';
        extension_settings.sd.reference_caption = '';
        $('#sd_gen_reference').val('');
        $('#sd_gen_reference_preview').attr('src', '').hide();
        $('#sd_gen_reference_caption').val('');
        $('#sd_gen_reference_status').text('Reference cleared');
        saveSettingsDebounced();
    });
    $('#sd_gen_reference_analyze').on('click', async function () {
        if (!extension_settings.sd.reference_image) {
            toastr.warning('请先上传参考图。', '图像分析');
            return;
        }
    ...
    if (extension_settings.sd.reference_caption) {
        $('#sd_gen_reference_status').text('Reference analysis ready');
    }
    $('#sd_gen_minimize').on('click', () => configPanel.toggleClass('minimized'));
    $('#sd_gen_generate').on('click', () => {
        const protagonistPrompt = String(extension_settings.sd.protagonist_override || '').trim();
        const scenePrompt = String(extension_settings.sd.scene_cast_override || '').trim();
        const sourceParts = [];
        if (protagonistPrompt) sourceParts.push(`Authoritative protagonist setting:\n${protagonistPrompt}`);
        if (scenePrompt) sourceParts.push(`Authoritative scene cast setting:\n${scenePrompt}`);
        if (extension_settings.sd.source_mode !== 'interface_text') {
            const card = getCharacterCardPrompt();
            if (card) sourceParts.push(`Character card details:\n${card}`);
        }
        try {
            sourceParts.push(`Current story context:\n${getLatestStoryParagraphs().join('\n')}`);
        } catch (error) {
            console.debug('No story context available for direct reference generation:', error);
        }
        const prompt = [
            'Create an image using the uploaded reference image for visual identity and appearance.',
            'Follow the authoritative settings and current story context below. Do not replace the specified protagonist or setting with unrelated people or locations.',
            ...sourceParts,
        ].join('\n');
        generatePicture(initiators.wand, { quiet: 'false' }, prompt).catch(error => {
            console.error('Direct image generation failed:', error);
            toastr.error(error.message || String(error), 'Image Generation');
        });
    });

    extension_settings.sd.protagonist_override ??= '';
    extension_settings.sd.scene_cast_override ??= '';
    $('#sd_protagonist_override').val(extension_settings.sd.protagonist_override).on('input', function () {
        extension_settings.sd.protagonist_override = String($(this).val());
        saveSettingsDebounced();
    });
    $('#sd_scene_cast_override').val(extension_settings.sd.scene_cast_override).on('input', function () {
    ...
    $('.sd_gen_action').on('click', function () {
        ...
        if (trigger) {
            ...
            generatePicture(initiators.wand, {}, trigger, undefined, appendGeneratedImageToLastMessage).catch(error => {
                console.error('SD button generation failed:', error);
                toastr.error(error.message || String(error), 'Image Generation');
            });
        }
    });
...
async function appendGeneratedImageToLastMessage(prompt, image, generationType, additionalNegativePrefix, _initiator, _prefixedPrompt, format) {
    const context = getContext();
    const messageId = context.chat.length - 1;
    if (messageId < 0 || context.chat[messageId]?.is_user !== false || !Array.isArray(context.chat[messageId]?.extra?.media)) {
        return sendMessage(prompt, image, generationType, additionalNegativePrefix, initiators.wand, '', format);
    }
    const message = context.chat[messageId];
    message.extra.media.push({ url: image, type: isVideo(format) ? MEDIA_TYPE.VIDEO : MEDIA_TYPE.IMAGE, title: prompt, generation_type: generationType, negative: additionalNegativePrefix, source: MEDIA_SOURCE.GENERATED });
...
function isValidState() {
    switch (extension_settings.sd.source) {
        ...
        case sources.openai:
            // The OpenAI-compatible relay is also used by the image endpoint.
            return secret_state[SECRET_KEYS.OPENAI] || secret_state[SECRET_KEYS.CUSTOM];
        case sources.aimlapi:
