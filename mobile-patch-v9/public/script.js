...
export async function updateSwipeCounter(mesId, { message = undefined, messageElement = undefined } = {}) {
    ...
    // Repair stale swipe ids left by an interrupted historical swipe.
    if (message?.swipes?.length) {
        const maxSwipeId = message.swipes.length - 1;
        if (!Number.isInteger(message.swipe_id) || message.swipe_id > maxSwipeId) {
            message.swipe_id = maxSwipeId;
        }
    }
...
export function isMessageSwipeable(messageId, message = undefined) {
    ...
    if (
        ...
        //Assistant messages in any paragraph can be swiped. The stock UI
        //restricted this to the last message, making historical counters
        //non-interactive even when their buttons were visible.
        (message &&
...
export function getOverswipeBehavior(messageId, message = undefined) {
    ...
    // Historical paragraphs use the same right-arrow action to create a new
    // reply. The swipe handler temporarily makes that paragraph the tail
    // while generation runs, then restores the following messages.
    if (messageId !== chat.length - 1 && message?.extra?.swipeable !== false) {
    ...
    if (direction === SWIPE_DIRECTION.LEFT) {
        ...
        if (newSwipeId >= chat[mesId].swipes.length) {
            ...
            if (overswipe == OVERSWIPE_BEHAVIOR.NONE) {
                ...
                const historicalTail = mesId < chat.length - 1 ? chat.splice(mesId + 1) : null;
                //Generate.
                try {
                    ...
                    if (historicalTail) {
                        chat.splice(mesId + 1, 0, ...historicalTail);
                        await redisplayChat({ startIndex: mesId });
                    }
                }
                return;
...
jQuery(async function () {
    ...
    // Allow every assistant paragraph to browse or regenerate its own replies.
    // The stock handlers only target .last_mes, which makes historical
    // paragraphs' counters effectively read-only.
